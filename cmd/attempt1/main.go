...
func t1() {
    ...
    // read and tokenise chunks
    tokensChan, tokensWg := CreateWorkerGroup[StationToken]()
    tokensWg.Add(channelSize)
    for range channelSize {
        go readChunk(chunkChan, tokensChan, tokensWg)
    }
    go wait(masterWg, tokensWg, tokensChan)

    // collect chunks
    tokenMapChan, tokenMapWg := CreateWorkerGroup[StationDataMap]()
    tokenMapWg.Add(1)
    go collectTokens(tokensChan, tokenMapChan, tokenMapWg)
    go wait(masterWg, tokenMapWg, tokenMapChan)

    masterWg.Wait()

    for t := range tokenMapChan {
        fmt.Printf("t: %v\n", t)
    }
